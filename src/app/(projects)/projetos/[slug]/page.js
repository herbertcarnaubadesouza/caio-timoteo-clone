...
const ProjectPage = () => {
  ...
  const project = params.slug
    ? projects.find((project) => project.searchableKey === `/${params.slug}`)
    : null;
  ...
  return (
    <section className="all-content">
      <Head>
        <title>{`${project.projectName} - ${metadata.title}`}</title>
        <meta name="description" content={metadata.description} />
      </Head>
      <main className="content-page">
        <SectionVideo videoUrl={project.projectVideoUrl} />
        <DataSheet project={project} />
        <SectionSlider pictures={project.projectPictures} />
      </main>
    </section>
  );
};
